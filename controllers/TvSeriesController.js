...
var listAllCount = function (res, params) {
    ...
    TvSeries.find(listAllFilter(params)).count(function (err, total) {
...
var save = function (req, res) {
    ...
    if (req.body._id !== undefined) {
        ...
        obj = getParamsTvSeries(req, null);
...
var remove = function (req, res, obj, step) {
    ...
    if(step == 1){
        ...
        GLOBAL.db_open();
        TvSeries.findOne(({ _id: req.body.id }), function (err, tv_series) {
    ...
    // DELETE ALL VIDEOS
        ...
        try{ 
            ...
            TvSeriesVideo.remove({ tv_serie_id: obj._id }, function (err) {
                GLOBAL.db_close();
    ...
    // DELETE ALL LIST
        ...
        try{ 
            ...
            TvSeriesList.remove({ tv_serie_id: obj._id }, function (err) {
                GLOBAL.db_close();
    ...
    // DELETE TV SERIES
        ...
        try{ 
            ...
            TvSeries.remove({ _id: obj._id }, function (err) {
                    GLOBAL.db_close();
        ...
        try {
            ...
            if (obj.lists) {
                for (var i = 0; i < obj.lists.length; i++) {
                    if (obj.lists[i].thumb) {
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES_LIST + path.basename(obj.lists[i].thumb));
                    }
                }
            }
            ...
            if (obj.thumb) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES + path.basename(obj.thumb));
            }
            // DELETE MAIN COVER
            if (obj.cover) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES_COVER + path.basename(obj.cover));
            }
            // DELETE MAIN BACKGROUND
            if (obj.background) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES_BACKGROUND + path.basename(obj.background));
            }
...
// GET ALL LIST THAT IT HAAS A RELATION WITH THIS TV SERIES
var getArrayList = function (tv_series_id, callback) {
    GLOBAL.db_open();
    
    require('./../models/TvSeries.list');
    var TvSeriesList = GLOBAL.mongoose.model('TvSeriesList');
    
    TvSeriesList.find({tv_series_id : tv_series_id})
    .exec(
...
// GET ALL VIDEOS THAT IT HAAS A RELATION WITH THIS TV SERIES
var getArrayVideo = function (tv_series_id, callback) {
    GLOBAL.db_open();
    
    require('./../models/TvSeries.video');
    var TvSeriesVideo = GLOBAL.mongoose.model('TvSeriesVideo');
    
    TvSeriesVideo.find({tv_series_id : tv_series_id})
    .exec(
...
// ======================================= =======================================
var getParamsTvSeries = function (req, obj) {
...
var update = function (id, obj, res) {
    ...
    TvSeries.findByIdAndUpdate(id, obj, function (err, item) {
...
var findOneCB = function (id, callback, callParams) {
    ...
    GLOBAL.db_open();
    TvSeries.findOne({ _id: id }, function (err, item) {
...
var getAndSetMainImages = function (callParams) {
    ...
    if (callParams.img_step == 1) {
        ...
        if (thumb_file) {
            ...
            GLOBAL.saveFile(thumb_file, thumbName, GLOBAL.path.SERVER_TVSERIES, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if(callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES + path.basename(callParams.old_obj.thumb));

                    callParams.obj.thumb = GLOBAL.path.WEB_TVSERIES + thumbName;
                }
                getAndSetMainImages(callParams);
            ...
            GLOBAL.download(callParams.obj.thumb, (GLOBAL.path.SERVER_TVSERIES + thumbName), function (e) {
    ...
    // SAVE COVER
        ...
        if (cover_file) {
            ...
            GLOBAL.saveFile(cover_file, thumbName, GLOBAL.path.SERVER_TVSERIES_COVER, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if (callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES_COVER + path.basename(callParams.old_obj.cover));

                    callParams.obj.cover = GLOBAL.path.WEB_TVSERIES_COVER + thumbName;
                }
                getAndSetMainImages(callParams);
    ...
    // SAVE BACKGROUND
        ...
        if (background_file) {
            ...
            GLOBAL.saveFile(background_file, thumbName, GLOBAL.path.SERVER_TVSERIES_BACKGROUND, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if (callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_TVSERIES_BACKGROUND + path.basename(callParams.old_obj.background));

                    callParams.obj.background = GLOBAL.path.WEB_TVSERIES_BACKGROUND + thumbName;
                }
