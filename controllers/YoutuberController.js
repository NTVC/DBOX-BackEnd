...
var listAllCount = function (res, params) {
    ...
    Youtuber.find(listAllFilter(params)).count(function (err, total) {
...
var save = function (req, res) {
    ...
    if (req.body._id !== undefined) {
        ...
        obj = getParamsYoutuber(req, null);
...
var remove = function (req, res, obj, step) {
    ...
    if(step == 1){
        ...
        GLOBAL.db_open();
        Youtuber.findOne(({ _id: req.body.id }), function (err, youtuber) {
    ...
    // DELETE ALL VIDEOS
        ...
        try{ 
            ...
            YoutuberVideo.remove({ youtuber_id: obj._id }, function (err) {
                GLOBAL.db_close();
    ...
    // DELETE ALL LIST
        ...
        try{ 
            ...
            YoutuberList.remove({ youtuber_id: obj._id }, function (err) {
                GLOBAL.db_close();
    ...
    // DELETE YOUTUBER
        ...
        try{ 
            ...
            Youtuber.remove({ _id: obj._id }, function (err) {
                    GLOBAL.db_close();
        ...
        try {
            ...
            if (obj.lists) {
                for (var i = 0; i < obj.lists.length; i++) {
                    if (obj.lists[i].thumb) {
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER_LIST + path.basename(obj.lists[i].thumb));
                    }
                }
            }
            ...
            if (obj.thumb) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER + path.basename(obj.thumb));
            }
            // DELETE MAIN COVER
            if (obj.cover) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER_COVER + path.basename(obj.cover));
            }
            // DELETE MAIN BACKGROUND
            if (obj.background) {
                GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER_BACKGROUND + path.basename(obj.background));
            }
...
// ======================================= =======================================  
// GET ALL LIST THAT IT HAS A RELATION WITH THIS YOUTUBER
// ======================================= =======================================
var getArrayList = function (youtuber_id, callback) {
    ...
    YoutuberList.find({youtuber_id : youtuber_id})
    .exec(
...
// ======================================= =======================================  
// GET ALL VIDEOS THAT IT HAS A RELATION WITH THIS YOUTUBER
// ======================================= =======================================
var getArrayVideo = function (youtuber_id, callback) {
    GLOBAL.db_open();
    
    require('./../models/Youtuber.video');
    var YoutuberVideo = GLOBAL.mongoose.model('YoutuberVideo');
    
    YoutuberVideo.find({youtuber_id : youtuber_id})
    .exec(
...
// ======================================= =======================================
var getParamsYoutuber = function (req, obj) {
...
var update = function (id, obj, res) {
    ...
    Youtuber.findByIdAndUpdate(id, obj, function (err, item) {
...
var findOneCB = function (id, callback, callParams) {
    ...
    GLOBAL.db_open();
    Youtuber.findOne({ _id: id }, function (err, item) {
...
var getAndSetMainImages = function (callParams) {
    ...
    if (callParams.img_step == 1) {
        ...
        if (thumb_file) {
            ...
            GLOBAL.saveFile(thumb_file, thumbName, GLOBAL.path.SERVER_YOUTUBER, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if(callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER + path.basename(callParams.old_obj.thumb));

                    callParams.obj.thumb = GLOBAL.path.WEB_YOUTUBER + thumbName;
                }
                getAndSetMainImages(callParams);
    ...
    // SAVE COVER
        ...
        if (cover_file) {
            ...
            GLOBAL.saveFile(cover_file, thumbName, GLOBAL.path.SERVER_YOUTUBER_COVER, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if (callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER_COVER + path.basename(callParams.old_obj.cover));

                    callParams.obj.cover = GLOBAL.path.WEB_YOUTUBER_COVER + thumbName;
                }
                getAndSetMainImages(callParams);
    ...
    // SAVE BACKGROUND
        ...
        if (background_file) {
            ...
            GLOBAL.saveFile(background_file, thumbName, GLOBAL.path.SERVER_YOUTUBER_BACKGROUND, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if (callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_YOUTUBER_BACKGROUND + path.basename(callParams.old_obj.background));

                    callParams.obj.background = GLOBAL.path.WEB_YOUTUBER_BACKGROUND + thumbName;
                }
