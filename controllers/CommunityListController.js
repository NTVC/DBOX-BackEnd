...
exports.process = function (req, res) {
    ...
    // ---------------------------------------------
    // LIST ALL BY COMMUNITY ID
    if (GLOBAL.getBoolean(req.query.getList) && req.query.community_id != undefined) {
    ...
    // ---------------------------------------------
    // SAVE OR UPDATE LIST OF COMMUNITY
...
// LIST ALL COMMUNITYS ON THE DATABASE
var getArrayList = function (res, params) {
    GLOBAL.db_open();
    
    CommunityList.find(listAllFilter(params))
    .sort({registration: -1})
...
var getArrayListCount = function (res, params) {
    ...
    CommunityList.find(listAllFilter(params)).count(function (err, total) {
...
var saveByStep = function (callParams){
    ...
    if (callParams.step == 1) {
        ...
        if (callParams.req.body._id !== undefined) {
            ...
            callParams.obj = new CommunityList();
    ...
    // CHECK THUMBNAIL
        ...
        if (thumb_file) {
            ...
            GLOBAL.saveFile(thumb_file, thumbName, GLOBAL.path.SERVER_COMMUNITY_LIST, function (r, thumbName) {
                
                if (r) {
                    // DELETE OLD FILE
                    if (callParams.old_obj)
                        GLOBAL.deleteFile(GLOBAL.path.SERVER_COMMUNITY_LIST + path.basename(callParams.old_obj.thumb));

                    callParams.obj.thumb = GLOBAL.path.WEB_COMMUNITY_LIST+ thumbName;
                }
                saveByStep(callParams);
    ...
    // SAVE / UPDATE
        ...
        if (callParams.req.body._id !== undefined) {
            ...
            CommunityList.findByIdAndUpdate(callParams.req.body._id, callParams.obj, function (err) {
...
var findOneCB = function (id, callback, callParams) {
    ...
    GLOBAL.db_open();
    CommunityList.findOne({ _id: id }, function (err, item) {
...
// REMOVE LIST OF COMMUNITY FROM DATABASE
var remove = function (req, res) {
    
    GLOBAL.db_open();
    
    CommunityList.findOne(({ _id: req.body._id }), function (err, list) {
        
        CommunityList.remove({ _id: req.body._id }, function (err) {
            ...
            if (err) {
                ...
                if (list.thumb) {
                    GLOBAL.deleteFile(GLOBAL.path.SERVER_COMMUNITY_LIST + path.basename(list.thumb));
